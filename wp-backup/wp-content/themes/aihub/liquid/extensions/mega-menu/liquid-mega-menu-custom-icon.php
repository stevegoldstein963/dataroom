...
class Liquid_Menu_Item_Custom_Image {
	...
	public function menu_image_nav_menu_manage_columns( $columns ) {
		return $columns + array( 'image' => esc_html__( 'Image', 'aihub' ) );
	}
	...
	public function wp_post_thumbnail_only_html( $item_id ) {
		...
		$content      = sprintf(
			$markup,
			esc_html__( 'Custom Image/svg Icon', 'aihub' ),
			$thumbnail_id ? esc_attr__( 'Change menu item image/svg icon', 'aihub' ) : esc_attr__( 'Set menu item image/svg icon', 'aihub' ),
			'',
			$item_id,
			$thumbnail_id ? wp_get_attachment_image( $thumbnail_id, $default_size ) : esc_html__( 'Set image/svg icon', 'aihub' ),
			$thumbnail_id ? '<a href="#" class="remove-post-thumbnail">' . esc_html__( 'Remove', 'aihub' ) . '</a>' : ''
		);
