 ...
 */
final class Liquid_Menu_Icons_Front_End {
	...
	public static function init() {
		$active_types = Liquid_Menu_Icons_Settings::get( 'global', 'icon_types' );

		if ( empty( $active_types ) ) {
			return;
		}

		foreach ( Liquid_Menu_Icons::get( 'types' ) as $type ) {
	...
	public static function _enqueue_styles() {
		...
		if ( $wp_styles && isset( $wp_styles->registered['font-awesome'] ) ) {
			...
			if ( strpos( $registered->src, Liquid_Menu_Icons::get( 'url' ) ) !== false ) {
				$wp_styles->remove( 'font-awesome' );
				$registered->ver = Liquid_Menu_Icons_Font_Awesome::$version;
	...
	public static function _add_icon( $title, $id ) {
		$meta = Liquid_Menu_Icons_Meta::get( $id );
	...
	public static function get_icon_classes( $meta, $output = 'string' ) {
		$classes = array( 'lqd-menu-icon' );

        $classes[] = "lqd-menu-icon-{$meta['position']}";
	...
	public static function get_font_icon( $meta ) {
		...
		// Force Render SVG
		if ( in_array( $type, [ 'fa', 'dashicons', 'akar-icons' ] ) ) {
			return Liquid_Menu_Icons_Render_SVG::build_svg( $type, $icon, $meta );
		}
