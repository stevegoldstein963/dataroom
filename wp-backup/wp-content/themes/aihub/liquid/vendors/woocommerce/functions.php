...
if ( !function_exists( 'liquid_get_secondary_product_image' ) ) {
	function liquid_get_secondary_product_image() {
		...
		if( ! empty( $hover_img_id ) ) {
			...
			echo '<figure class="ld-sp-img-hover mt-0 mb-0 absolute top-0 start-0"><a href="' . get_permalink() . '">' . $image . '</a></figure>';
...
if ( ! function_exists( 'liquid_woocommerce_get_product_thumbnail' ) ) {
    ...
    function liquid_woocommerce_get_product_thumbnail( $size = 'woocommerce_thumbnail', $deprecated1 = 0, $deprecated2 = 0 ) {
		...
		if( ! empty( $hover_img_id ) ) {
			$hover_img = '<figure class="ld-sp-img-hover mt-0 mb-0 absolute top-0 start-0">' . wp_get_attachment_image( $hover_img_id, $image_size, false, array( 'class' => 'w-full h-full objfit-cover objfit-center', 'alt' => esc_attr( $product->get_title() ), 'class' => 'invisible' ) ) . '</figure>' ;
		}
...
if ( ! function_exists( 'liquid_woocommerce_product_classnames' ) ) {
	function liquid_woocommerce_product_classnames() {
		...
		if( 'classic' === $style ) {
			echo 'ld-sp-clsc pb-24 text-center';
		}
		elseif( 'classic-alt' === $style ) {
			echo 'ld-sp-clsc ld-sp-clsc-alt pb-32 text-center';
		}
