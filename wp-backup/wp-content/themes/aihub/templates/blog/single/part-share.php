...
$sticky_share_view = liquid_helper()->get_page_option( 'post_floating_box_social_style' );
$sticky_share_view = $sticky_share_view ? $sticky_share_view : liquid_helper()->get_kit_frontend_option( 'liquid_blog_single_floating_box_social_style' );
$author_in_sticky = liquid_helper()->get_page_option( 'post_floating_box_author_enable' );
...
if ( $sticky_share_view === 'with-text-outline' ) {
	$social_icons_classname .= 'p-0 m-0 list-none';
} else {
	$social_icons_classname .= 'p-0 m-0 list-none';
}
...
<div class="lqd-post-sticky-stuff">
	<div class="lqd-post-sticky-stuff-inner">
		...
		<div class="lqd-post-share">
			...
			<span><?php esc_html_e( 'Share', 'aihub' ); ?></span>
			<?php endif; ?>
			<ul class="<?php echo liquid_helper()->sanitize_html_classes( $social_icons_classname ); ?>">
				<li>
					<a rel="nofollow" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?php the_permalink(); ?>">
						...
						<?php if ( $sticky_share_view === 'with-text-outline' ) echo '<span style="margin-inline-start: 1em;">' . esc_html__( 'Facebook', 'aihub' ) . '</span>' ?>
					</a>
				</li>
				<li>
					<a rel="nofollow" target="_blank" href="https://twitter.com/intent/tweet?text=<?php echo urlencode( get_the_title() ); ?>&amp;url=<?php the_permalink(); ?>">
						...
						<?php if ( $sticky_share_view === 'with-text-outline' ) echo '<span style="margin-inline-start: 1em;">' . esc_html__( 'Twitter', 'aihub' ) . '</span>' ?>
					</a>
				</li>
				<li>
					<a rel="nofollow" target="_blank" href="https://pinterest.com/pin/create/button/?url=&amp;media=<?php echo esc_url( $pinterest_image ); ?>&amp;description=<?php echo urlencode( get_the_title() ); ?>">
						...
						<?php if ( $sticky_share_view === 'with-text-outline' ) echo '<span style="margin-inline-start: 1em;">' . esc_html__( 'Pinterest', 'aihub' ) . '</span>' ?>
					</a>
				</li>
				<li>
					<a rel="nofollow" target="_blank" href="https://www.linkedin.com/shareArticle?mini=true&url=<?php the_permalink(); ?>&amp;title=<?php echo get_the_title(); ?>&amp;source=<?php echo get_bloginfo( 'name' ); ?>">
						...
						<?php if ( $sticky_share_view === 'with-text-outline' ) echo '<span style="margin-inline-start: 1em;">' . esc_html__( 'Linkedin', 'aihub' ) . '</span>' ?>
					</a>
				</li>
			</ul>
		</div>
