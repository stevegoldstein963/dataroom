...
class LQD_Text extends Widget_Base {
	...
	private function get_highlight_fill_markup( $item, $id ) {
		...
		if ( $color_type === 'gradient' ) {
			...
			$angle = isset( $gradient_angle_size ) && !empty( $gradient_angle_size ) ? $gradient_angle_size : 0;
		...
		$svg_markup = sprintf(
			'<svg width="0" height="0" version="1.1" xmlns="http://www.w3.org/2000/svg">
				<defs>
					<linearGradient id="%1$s" x1="%2$s" y1="%2$s" x2="%3$s" y2="%4$s">
						%6$s
					</linearGradient>
				</defs>
			</svg>',
	...
	private function get_image_markup( $item ) {
		...
		if ( $split_type === 'words' ) {
			$image = sprintf( '<span class="lqd-split-text-words inline-flex">%1$s</span>', $image );
		} elseif ( $split_type === 'chars,words' ) {
			$image = sprintf( '<span class="lqd-split-text-words inline-flex"><span class="lqd-split-text-chars inline-flex">%s</span></span>', $image );
		}
		...
		if ( $split_type === 'words' ) {
			...
			foreach ( $words as $key => $word ) {
				...
				$splitted_text .= sprintf(
					'%2$s<span class="lqd-split-text-words inline-flex bg-inherit relative">%1$s</span>',
