    ...
    public function set_demo_theme_options( $file = '' ) {
      if(empty($file)) {
        ...
        $this->log->putContent('Faild to import theme options', true, true);
  			wp_die(
  				 esc_html__( 'Theme options Import file could not be found. ', 'aihub-core' ),
  				'',
...
//var_dump($data);
        ...
        $this->log->putContent('Faild to import theme options', true, true);
  			wp_die(
  				esc_html__( 'Theme options import data could not be read. Please try a different file.', 'aihub-core' ),
  				'',
  				array( 'back_link' => true )
  			);
  		}else{
    ...
    function process_widget_import_file( $file = '') {
      ...
      if(empty($file)) { 
    	...
    	if ( ! file_exists( $file ) ) {
    		wp_die(
    			 esc_html__( 'Widget Import file could not be found. ', 'aihub-core' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    ...
    public function import_widgets( $data ) {
    	...
    	if ( empty( $data ) || ! is_object( $data ) ) {
    		wp_die(
    			esc_html__( 'Widget import data could not be read. Please try a different file.', 'aihub-core' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    	...
    	foreach ( $data as $sidebar_id => $widgets ) {
    		...
    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
    			...
    			$sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'aihub-core' );
    		}
    		...
    		foreach ( $widgets as $widget_instance_id => $widget ) {
    			...
    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
    				...
    				$widget_message = __( 'Site does not support widget', 'aihub-core' ); // explain why widget not imported
            $this->log->putContent('Site does not support widget', true, true);
    			...
    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
    				...
    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
    					...
    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
    						...
    						$widget_message = __( 'Widget already exists', 'aihub-core' ); // explain why widget not imported
    			...
    			if ( ! $fail ) {
    				...
    				if ( $sidebar_available ) {
    					...
    					$widget_message = esc_html__( 'Widgets Imported', 'aihub-core' );
						$this->log->putContent('Widgets Imported', true, true);
    				} else {
    					$widget_message_type = 'warning';
    					$widget_message = esc_html__( 'Imported to Inactive', 'aihub-core' );
    				}
    			...
    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'aihub-core' ); // show "No Title" if widget instance is untitled
